...
def evaluate_model(sub, eeg_model, dataloader, device, text_features_all, img_features_all, config, epoch, k):
    ...
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            all_img_features.append(img_features.cpu().numpy())
    ...
    all_eeg_features = np.concatenate(all_eeg_features, axis=0)
    all_img_features = np.concatenate(all_img_features, axis=0) 
    # all_labels = np.concatenate(all_labels, axis=0) all_subject_ids = np.concatenate(all_subject_ids, axis=0)
    ...
    n_subjects = max_samples // samples_per_subject
    subj_ids = np.repeat(np.arange(n_subjects), samples_per_subject)
    # n_subjects = 6
    # subj_ids = 10 * [4, 5, 1, 0, 3, 1, 0, 0, 1, 1, 3, 5, 1, 4, 3, 1, 4, 3, 0, 1, 1, 3, 3, 5, 4, 0, 3, 0, 3, 1, 4, 3, 5, 5, 1, 1, 2, 3, 5, 1, 1, 1, 1, 1, 2, 5, 3, 3, 1, 3, 3, 2, 5, 3, 3, 1, 3, 5, 4, 1, 3, 1, 5, 3, 5, 0, 1, 1, 5, 5, 3, 5, 5, 0, 4, 5, 0, 1, 1, 1, 3, 3, 2, 4, 4, 5, 5, 4, 5, 0, 3, 1, 1, 2, 1, 2, 4, 3, 0, 4, 3, 0, 1, 2, 1, 4, 1, 1, 3, 4, 4, 3, 3, 1, 4, 1, 4, 1, 1, 3, 1, 3, 3, 3, 3, 0, 4, 2, 4, 1, 4, 3, 4, 1, 3, 4, 4, 2, 1, 3, 3, 4, 3, 4, 1, 2, 3, 3, 1, 4, 0, 4, 2, 4, 2, 0, 3, 3, 3, 1, 4, 0, 3, 4, 4, 1, 0, 0, 2, 1, 1, 4, 1, 3, 4, 2, 1, 0, 1, 1, 2, 4, 3, 1, 1, 2, 1, 2, 2, 4, 4, 1, 0, 2, 3, 3, 1, 4, 3, 1]
    eeg_sub = all_eeg_features[:max_samples]
    img_sub = all_img_features[:max_samples]
    subj_sub = subj_ids[:max_samples]
    subj_sub = np.array(subj_sub)

    # ---------- EEG t-SNE ----------
    tsne_eeg = TSNE(n_components=2, perplexity=5, learning_rate=2000, random_state=40)
    eeg_result = tsne_eeg.fit_transform(eeg_sub)
    colors = plt.cm.tab10(np.linspace(0, 1, n_subjects))  # 10 个颜色
    plt.figure(figsize=(10, 8))
    for i in range(n_subjects):
        idx = np.where(subj_sub == i)[0]
        plt.scatter(
            eeg_result[idx, 0], eeg_result[idx, 1],
            color=colors[i],  # 每个 subject 固定一个颜色
            label=f"Category {i}",
            alpha=0.7, s=20
        )
    plt.legend()
    plt.title("t-SNE of EEG Features (2000 samples)")
    plt.savefig(os.path.join(save_path, f"tsne_eeg_epoch{epoch}.png"))
    plt.close()

    # ---------- Image t-SNE ----------
    tsne_img = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=40)
    img_result = tsne_img.fit_transform(img_sub)
    colors = plt.cm.tab10(np.linspace(0, 1, n_subjects))  # 10 个颜色
    plt.figure(figsize=(10, 8))
    for i in range(n_subjects):
        idx = np.where(subj_sub == i)[0]
        plt.scatter(
            img_result[idx, 0], eeg_result[idx, 1],
            color=colors[i],  # 每个 subject 固定一个颜色
            label=f"Category {i}",
            alpha=0.7, s=20
        )
    plt.legend()
    plt.title("t-SNE of Image Features (2000 samples)")
    plt.savefig(os.path.join(save_path, f"tsne_img_epoch{epoch}.png"))
    plt.close()
...
def main():
    ...
    for sub in subjects:
        ...
        eeg_model.to(device)
        path =r"D:\fzh\15-EEG\models\contrast\across\NeuralMCRL-eeg-sub-01-09-18_17-01\best_top5-0.7260.pth"
        ...
        if args.insubject:
            ...
            test_dataset = EEGDataset(args.data_path, subjects=subjects, train=False)
